...
import { ApiKeyModal } from './components/ApiKeyModal'; // New Component
...
import { Leaf, Cpu, Zap, ArrowRight, Activity, Info, Database, Lightbulb, AlertTriangle, CheckCircle2, HelpCircle, ShieldCheck, Share2, Lock, EyeOff, BookOpen, ChevronDown, BrainCircuit, Search, Server, FileText, Bug, Settings, Download, DollarSign } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Prefer environment variable (Dev), fallback to stored key (Prod/MVP)
    ...
    if (envKey) {
        ...
        // Delay modal slightly for animation smoothness
...
*Generated by EcoCompute AI using Gemini 3*
  ...
  return (
    <div className="min-h-screen bg-[#11111b] text-gray-200 font-sans selection:bg-eco-500/30 flex flex-col">
      ...
      <ApiKeyModal isOpen={showKeyModal} onSave={handleSaveKey} onClose={() => setShowKeyModal(false)} hasKey={!!apiKey} />
            ...
            {isAnalyzing ? (
              ...
              <div className="h-full rounded-2xl border-2 border-dashed border-gray-800 bg-dark-800/30 flex flex-col items-center justify-center text-gray-500 gap-4 p-8">
                ...
                <div className="text-center">
                  ...
                  {!apiKey && (
                       <button 
                         onClick={() => setShowKeyModal(true)}
                         className="mt-6 px-5 py-2 rounded-lg bg-eco-600/20 text-eco-400 border border-eco-500/50 hover:bg-eco-600 hover:text-white transition-all text-sm font-bold"
                       >
              ...
              <>
                <div className="flex flex-col gap-2 animate-in fade-in slide-in-from-bottom-4">
                  ...
                  {/* Transparency & Assumptions (Collapsible Panel) */}
                ...
                {/* Cloud FinOps Card - New for Ranking Boost */}
